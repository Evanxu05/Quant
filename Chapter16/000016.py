...
df_stockload = ts.pro_bar(ts_code=stock_code, asset='I', start_date=Start_date.strftime('%Y%m%d'), end_date=End_date.strftime('%Y%m%d'),freq='15min',ma=[8, 16, 40,80])
...
graph_KAV.plot(numt, df_stockload['ma40'],'blue',label='M40', lw=1.0)
graph_KAV.plot(numt, df_stockload['ma80'],'yellow',label='M80', lw=1.0)
...
#显示MA40均线转正/转负提示符
list_diff = np.sign(df_stockload['ma40']-df_stockload['ma40'].shift(1))
...
for i in range(len(list_signal)):
    if list_signal[i] < 0:
        graph_KAV.annotate(u"转负", xy=(i, df_stockload['ma40'][i]), xytext=(i, df_stockload['ma40'][i]+1.5),
                         arrowprops=dict(facecolor='green', shrink=0.2))
        print(df_stockload.iloc[i])
    if list_signal[i] > 0:
        graph_KAV.annotate(u"转正", xy=(i, df_stockload['ma40'][i]), xytext=(i, df_stockload['ma40'][i]-1.5),
...
graph_MACD.plot(np.arange(0, len(df_stockload.index)), macd_dif, 'red', label='macd dif16') #dif    
...
macd_dif2, macd_dea2, macd_bar2 = talib.MACD(df_stockload['close'].values, fastperiod=80, slowperiod=160, signalperiod=60)
graph_MACD2.plot(np.arange(0, len(df_stockload.index)), macd_dif2, 'red', label='macd dif80') #dif    
